# 小爱-Hermes 桥接服务 完整部署教程

> 让小爱音箱成为你的语音 Agent 入口 —— 对小爱说话，Hermes 回答，小爱播报。
...
**小爱-Hermes 桥接服务** 是一个 Python 服务，把小爱音箱接入 Hermes Agent，实现：

- 对小爱音箱说话 → 语音被识别 → 转发给 Hermes LLM → 回复通过小爱播报
- 通过 Web 控制台管理设备、切换模式、查看日志
- 支持唤醒词拦截、代理模式、静默模式

### 项目来源

本项目功能对标 [Xiaoai-Claw-Addon](https://github.com/ZhengXieGang/Xiaoai-Claw-Addon)（OpenClaw 插件），使用 Python 重新实现，适配 Hermes Agent。
...
| 小米密码 | 账号密码 |
| 小爱音箱 | 需要已绑定到你的小米账号 |
| Hermes API | 需要 Hermes API Server 运行中 (`hermes serve`) |
...
```yaml
# 必填：Hermes API 地址
hermes_api_url: "http://127.0.0.1:9222"
...
```yaml
# Hermes API 地址（需要先启动 hermes serve）
hermes_api_url: "http://127.0.0.1:9222"
...
**重要：** 你需要先在另一个终端启动 Hermes API Server：
...
# 日志保存在 ~/.xiaomi-hermes-bridge/debug.log
```
...
Hermes API: http://127.0.0.1:9222
模型: mimo-v2.5
语音模式: wake
唤醒词: ^(贾维斯| Jarvis)
...
```bash
# 查看实时日志
tail -f ~/.xiaomi-hermes-bridge/debug.log
```
...
1. 确保 Hermes API Server 在运行：`hermes serve`
2. 启动桥接服务：`cd ~/xiaomi-hermes-bridge && python3 main.py`
3. 对小爱说"贾维斯 + 你的问题"
4. 等待回答通过小爱播报出来
...
```
用户: "小爱，今天天气"     → 拦截，转发到 Hermes
用户: "贾维斯你好"         → 拦截，转发到 Hermes
用户: "播放音乐"           → 拦截，转发到 Hermes（注意：不是真正的播放指令）
```
...
```bash
# 复制 service 文件
sudo cp ~/xiaomi-hermes-bridge/xiaomi-hermes.service /etc/systemd/system/
...
```bash
# 查看状态
sudo systemctl status xiaomi-hermes

# 查看日志
sudo journalctl -u xiaomi-hermes -f

# 重启
sudo systemctl restart xiaomi-hermes

# 停止
sudo systemctl stop xiaomi-hermes
```
...
```bash
# 编辑
sudo nano /etc/systemd/system/xiaomi-hermes.service
...
Environment=HERMES_API_URL=http://127.0.0.1:9222
```
...
# ─── Hermes API ───
hermes_api_url: "http://127.0.0.1:9222"  # API 地址
hermes_api_key: ""                        # API Key
hermes_model: "mimo-v2.5"                 # 模型名
...
├── README.md                  # 项目说明
├── xiaomi-hermes.service      # systemd 服务文件
│
├── xiaomi/                    # 小米 API 客户端
│   ├── __init__.py
│   ├── models.py              # 数据模型
...
# 2. 确认端口正确
# config.yaml 中 hermes_api_url 是否正确
```
...
| 特性 | Xiaoai-Claw-Addon (OpenClaw) | 本项目 (Hermes) |
|------|------|------|
...
A: 运行时的对话上下文在内存中，重启会清空。持久化的配置、登录态在 `~/.xiaomi-hermes-bridge/` 目录下。
...
A: 在代理模式下，所有语音都被转发到 Hermes，不会触发小爱原生的音乐播放。如果想让小爱播放音乐，用控制台的"执行"功能。
...
- [ ] `config.yaml` 已创建并配置
- [ ] Hermes API Server 正在运行 (`hermes serve`)
- [ ] `python3 main.py` 已启动
...
*文档版本: 2026-05-09*
*项目: xiaomi-hermes-bridge v1.0.0*
